...
**/
public class FFFBHeap<Val> : IEnumerable<KeyValuePair<Val, float>> {
	...
	public IEnumerator<KeyValuePair<Val, float>> GetEnumerator() {
		List<KeyValuePair<Val, float>> l = new List<KeyValuePair<Val, float>>();
		for (int i = 1; i <= size; i++) {
			l.Add(new KeyValuePair<Val, float>(vals[i], heap[i]));
		}
		return l.GetEnumerator();
		//vals.Skip(1).Take(size).GetEnumerator();
	}
