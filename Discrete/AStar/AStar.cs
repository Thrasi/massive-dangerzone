...
public class AStar {
	...
	// Graph to search on
	private readonly DiscreteMap gph;
	...
	public AStar(DiscreteMap gph, Vector3 start, Vector3 goal,
	...
	private void Astar() {
		...
		while (open.Count > 0) {
			...
			foreach (Tuple<Vector3, float> st in gph.Successors(curr.state)) {
				Vector3 s = st._1;
				Node temp = new Node(s, curr.g + st._2, h(s, goal), curr);
